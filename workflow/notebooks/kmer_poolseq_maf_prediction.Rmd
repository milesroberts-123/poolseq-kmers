...
```{r}
params = read.table("/global/scratch/users/milesroberts/poolseq-kmers/config/parameters.tsv", header = T)
...
kmer_pairs = read.table(paste("/global/scratch/users/milesroberts/poolseq-kmers/workflow/smudgeplot_results/", i, "_0_sequences.tsv", sep = ""), col.names = c("k1"))

kmer_covs = read.table(paste("/global/scratch/users/milesroberts/poolseq-kmers/workflow/smudgeplot_results/", i, "_0_coverages.tsv", sep = ""), col.names = c("c1", "c2"))
...
hetmers_seqs = read.csv(paste("/global/scratch/users/milesroberts/poolseq-kmers/workflow/hetmers_results/", i, "_0_seqs.csv", sep = ""), col.names = c("k1", "k2"))

hetmers_counts = read.csv(paste("/global/scratch/users/milesroberts/poolseq-kmers/workflow/hetmers_results/", i, "_0_counts.csv", sep = ""), col.names = c("c1", "c2"))

hetmers_bayes = read.csv(paste("/global/scratch/users/milesroberts/poolseq-kmers/workflow/hetmers_results/", i, "_0_bayes_states.csv", sep = ""), col.names = c("bc"))
...
#hetmers$hetmer_mac_bayes = pmin(hetmers$bc, 2*n - hetmers$bc)

# load in actual allele freqs from simulations
true_af = read.table(paste("/global/scratch/users/milesroberts/poolseq-kmers/workflow/slim_results/allele_freqs_", i, "_0.txt", sep = ""), col.names = c("chrom", "pos", "ref", "alt", "ns", "af", "ac"))
...
disco = read.table(paste("/global/scratch/users/milesroberts/poolseq-kmers/workflow/disco_results/", i, "_0.txt", sep = ""), col.names = c("chrom", "pos", "ref", "alt", "c1", "c2"))
...
pos_kmer_key = read.table(paste("/global/scratch/users/milesroberts/poolseq-kmers/workflow/seqkit_results/center_kmer_pairs_", i, ".txt", sep = ""), fill = T, col.names = c("pos", "k1", "k2", "k3", "k4"))
...
varscan = read.table(paste("/global/scratch/users/milesroberts/poolseq-kmers/workflow/varscan_results/", i, "_0.tsv", sep = ""), header = T)
...
# load poolsnp calls
poolsnp = read.table(paste("/global/scratch/users/milesroberts/poolseq-kmers/workflow/poolsnp_results/", i, "_0.txt", sep = ""), col.names = c("chrom", "pos", "ref", "alt", "c1", "t"), fill = T, na.strings = "")
...
write.table(pares, paste("/global/scratch/users/milesroberts/poolseq-kmers/results/", today, "/performance_metrics.txt",sep = ""), sep = " ")
```
...
plotdata_div = plotdata_div[(plotdata_div$variable %in% c("hetmer", "varscan", "poolsnp")),]
...
# benchmarks
```{r}
bench_file_names = list.files(path = "/mnt/scratch/robe1195/Josephs_Lab_Projects/poolseq-kmers/workflow/benchmarks", recursive = T, full.names = T)
...
bench_files$tool = gsub("/.*", "", gsub("/mnt/scratch/robe1195/Josephs_Lab_Projects/poolseq-kmers/workflow/benchmarks/","",bench_files$files))

bench_files$ID = as.numeric(gsub(".*/|.bench", "", gsub("/mnt/scratch/robe1195/Josephs_Lab_Projects/poolseq-kmers/workflow/benchmarks/","",bench_files$files)))
...
workflow_summary = function(x, lev_col, resource_col){
  ...
  results = NULL
  for(lev in levs){
    bwa = x[(x$tool == "bwa") & (x[,lev_col] == lev),resource_col]
    ...
    result = data.frame(
      lev_col = lev,
      bwa_varscan = bwa+varscan,
      bwa_poolsnp = bwa+poolsnp,
...
bench_summary_max_rss$tool = factor(bench_summary_max_rss$tool, levels = c("bwa", "varscan", "poolsnp", "kmc", "smudgeplot", "hetmers", "unitig-caller", "discosnp"))
