rule slim:
    ...
    shell:
        ...
        if [ "{params.simtype}" == "onepop" ]; then
            slim -d ID={wildcards.ID} -d SLIMSEED={params.slimseed} -d sigma={params.sigma} -d N={params.N} -d mu={params.mu} -d R={params.R} -d n={params.n} scripts/neutral.slim &> {log}
        fi

        if [ "{params.simtype}" == "twopop" ]; then
            slim -d ID={wildcards.ID} -d sigma={params.sigma} -d SLIMSEED={params.slimseed} -d N1={params.N1} -d N2={params.N2} -d mg1={params.mg1} -d mg2={params.mg2} -d mu={params.mu} -d R={params.R} -d n={params.n} -d tau={params.tau} scripts/two_pop.slim &> {log}
        fi

        if [ "{params.simtype}" == "bsa" ]; then
            slim -d ID={wildcards.ID} -d SLIMSEED={params.slimseed} -d N={params.N} -d mu={params.mu} -d R={params.R} -d n={params.n} -d qtl_mean={params.qtl_mean} -d qtl_sigma={params.qtl_sigma} -d qtl_prop={params.qtl_prop} -d optimum_mean={params.optimum_mean} -d optimum_sigma={params.optimum_sigma} -d phenotype_cutoff={params.phenotype_cutoff} scripts/bsa.slim &> {log}
        fi

        if [ "{params.simtype}" == "sweep" ]; then
            slim -d ID={wildcards.ID} -d SLIMSEED={params.slimseed} -d h={params.h} -d s={params.s} -d sigma={params.sigma} -d N={params.N} -d mu={params.mu} -d R={params.R} -d n={params.n} scripts/sweep.slim &> {log}
        fi
        """
