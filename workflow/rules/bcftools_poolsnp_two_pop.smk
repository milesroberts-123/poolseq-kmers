rule bcftools_poolsnp_two_pop:
    ...
    shell:
        ...
        bcftools view -m2 -M2 -v snps {input.vcf_p1} | bcftools query -f '%CHROM %POS %REF %ALT [ %AD] [ %DP]\n' | sed 's:,:\t:g' > {output.final_p1}
        bcftools view -m2 -M2 -v snps {input.vcf_p2} | bcftools query -f '%CHROM %POS %REF %ALT [ %AD] [ %DP]\n' | sed 's:,:\t:g' > {output.final_p2}
        """
