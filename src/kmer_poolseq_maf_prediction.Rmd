...
# filter kmers
high_nc = qnorm(0.9995, mean = nC, sd = sqrt(nC))
low_nc = qnorm(0.0005, mean = nC, sd = sqrt(nC))

high_nck = qnorm(0.9995, mean = nCk, sd = sqrt(nCk))
low_nck = qnorm(0.0005, mean = nCk, sd = sqrt(nCk))
...
filter = (kmer_pairs$c1 + kmer_pairs$c2 > low_nc) & (kmer_pairs$c1 + kmer_pairs$c2 < high_nc) & (kmer_pairs$c1 >= thresh) & (kmer_pairs$c2 >= thresh)

kmer_pairs = kmer_pairs[filter,]
...
disco = disco[(disco$c1 + disco$c2 > low_nc) & (disco$c1 + disco$c2 < high_nc) & (disco$c1 >= thresh) & (disco$c2 >= thresh),]
...
varscan = varscan[(varscan$Reads1 >= thresh) & (varscan$Reads2 >= thresh) & (varscan$Reads1 + varscan$Reads2 > low_nc) & (varscan$Reads1 + varscan$Reads2 < high_nc),]
...
poolsnp = poolsnp[(poolsnp$c1 >= thresh) & (poolsnp$t > low_nc) & (poolsnp$t < high_nc),]
