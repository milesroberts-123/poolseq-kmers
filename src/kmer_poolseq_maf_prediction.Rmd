...
hetmers_bayes = read.csv(paste("/mnt/scratch/robe1195/Josephs_Lab_Projects/poolseq-kmers/workflow/hetmers_", i, "_bayes_states.csv", sep = ""), col.names = c("b1", "b2"))
...
compare_maf_kmer = merge(true_af[,c("pos", "snp_maf", "ac")], pos_kmer_pairs, by = "pos", all.x = T, all.y = T)

compare_maf_hetmer = merge(true_af[,c("pos", "snp_maf", "ac")], pos_hetmers, by = "pos", all.x = T, all.y = T)
...
ggplot(plotdata[(plotdata$simtype == "onepop"),], aes(x = Ubar, y = mean_dist)) +
  geom_point() +
  geom_abline(slope = 1, intercept = 0) +
  theme_classic() +
...
ggplot(plotdata, aes(x = n, y = max_dist, color = as.factor(Nes))) +
  geom_boxplot() +
  theme_classic() +
...
names(my_colors) = unique(bench_summary_time_by_flow$lev_col)
...
# memory plots
plot_memory = ggplot(bench_summary_max_rss, aes(x = tool, y = max_rss, fill = as.factor(cov))) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(x = "Tool", y = "Average Maximum\nResident Set Size (MiB)", fill = "Coverage") +
  theme_classic() +
  theme(
    axis.line = element_line(colour = 'black', linewidth = 1),
    axis.ticks = element_line(colour = "black", linewidth = 1),
...
tpr_vs_coverage_plot = ggplot(tpr_plot_data, aes(x = cov, y = value, color = variable)) +
  ...
  labs(x = "Total coverage", y = "Sensitivity (true positive rate)", color = "Method", title = "All instruments")
...
fnr_plot_data = melt(pares[,c("ID", "cov", "n", "sequencer", "kmer_fnr", "disco_fnr", "varscan_fnr", "poolsnp_fnr")], id = c("ID", "cov", "n", "sequencer"))

fnr_plot_data$variable = factor(gsub("_fnr", "", fnr_plot_data$variable), levels = c("kmer", "disco", "varscan", "poolsnp"), ordered = T)
...
tnr_plot_data = melt(pares[,c("ID", "ID2", "cov", "n", "sequencer", "simtype", "kmer_tnr", "disco_tnr", "varscan_tnr", "poolsnp_tnr")], id = c("ID", "ID2", "cov", "n", "sequencer", "simtype"))

tnr_plot_data$variable = factor(gsub("_tnr", "", tnr_plot_data$variable), levels = c("kmer", "disco", "varscan", "poolsnp"), ordered = T)
...
tnr_vs_coverage_plot = ggplot(tnr_plot_data, aes(x = cov, y = value, color = variable)) +
  ...
  theme_classic() +
  theme(text = element_text(size = 12)) +
  labs(x = "Total coverage", y = "Specificity (true negative rate)", color = "Method", title = "All instruments")
...
ggsave(paste("../results/", today, "/coverage_vs_tpr_tnr_fpr_fnr.png", sep = ""), unit = "in", width = 8, height = 8, dpi = 350)
