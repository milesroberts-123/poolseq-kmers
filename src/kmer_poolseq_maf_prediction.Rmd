...
# filter kmers
high = qnorm(0.9995, mean = nC, sd = sqrt(nC))
low = qnorm(0.0005, mean = nC, sd = sqrt(nC))

ggplot(kmer_covs, aes(x = c1 + c2)) +
  geom_density() +
  geom_vline(xintercept = 2*thresh, lty = 3) + 
  geom_vline(xintercept = low, lty = 2) +
  geom_vline(xintercept = nC) +
  geom_vline(xintercept = high, lty = 2)

filter = (kmer_pairs$c1 + kmer_pairs$c2 > low) & (kmer_pairs$c1 + kmer_pairs$c2 < high) & (kmer_pairs$c1 >= thresh) & (kmer_pairs$c2 >= thresh)
...
disco = disco[(disco$c1 + disco$c2 > low) & (disco$c1 + disco$c2 < high) & (disco$c1 >= thresh) & (disco$c2 >= thresh),]
...
varscan = varscan[(varscan$Reads1 >= thresh) & (varscan$Reads2 >= thresh) & (varscan$Reads1 + varscan$Reads2 > low) & (varscan$Reads1 + varscan$Reads2 < high),]
...
poolsnp = poolsnp[(poolsnp$c1 >= thresh) & (poolsnp$t > low) & (poolsnp$t < high),]
