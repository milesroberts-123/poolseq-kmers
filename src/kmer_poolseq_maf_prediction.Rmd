...
hetmers = cbind(hetmers_seqs, hetmers_counts)
...
ggplot(plotdata, aes(x = n, y = mean_dist, color = s)) +
  geom_boxplot() +
  theme_classic() +
  labs(x = "Pool size", y = "Avg. distance between SNPs (bp)", color = "Selection", title = "Ground truth")

ggplot(plotdata, aes(x = n, y = max_dist, color = s)) +
  geom_boxplot() +
  theme_classic() +
...
ggplot(plotdata_unitigs, aes(x = sc, color = s)) +
  geom_density() +
  theme_classic() +
  geom_vline(xintercept = 2e6/2, lty = 2, lwd = 1.5) +
  labs(x = "Center of longest mapped unitig", color = "Simulation\ntype")
...
bench_summary_memory = aggregate(max_rss ~ tool + cov, data = bench_params, FUN = "sum")
...
bench_summary_memory_by_flow = workflow_summary(bench_summary_memory, "cov", "max_rss")
bench_summary_memory_by_flow = melt(bench_summary_memory_by_flow, id = "lev_col")

# calculate average times
bench_summary_time_by_flow$value = bench_summary_time_by_flow$value/length(unique(bench_params$ID))

bench_summary_memory_by_flow$value = bench_summary_memory_by_flow$value/length(unique(bench_params$ID))
...
# ggplot(bench_summary_memory_by_flow, aes(x = variable, y = value, fill = as.factor(lev_col))) +
#   geom_bar(stat = "identity", position = "dodge") +
#   labs(x = "workflow", y = "Average Maximum Resident Set Size (MB)", fill = "coverage") +
#   theme_classic() +
#   scale_fill_manual(values = my_colors) 

ggplot(bench_summary_memory, aes(x = tool, y = max_rss, fill = as.factor(cov))) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(x = "workflow", y = "Average Maximum Resident Set Size (MiB)", fill = "coverage") +
  theme_classic() +
  scale_fill_manual(values = my_colors) 
