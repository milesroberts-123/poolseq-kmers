...
for(i in simids){
  ...
  # load pairs of k-mers
  kmer_pairs_p1 = read.table(paste("/mnt/scratch/robe1195/Josephs_Lab_Projects/poolseq-kmers/workflow/hetmers_", i, "_p1_seqs.tsv", sep = ""), col.names = c("k1", "k2"))

  kmer_covs_p1 = read.table(paste("/mnt/scratch/robe1195/Josephs_Lab_Projects/poolseq-kmers/workflow/hetmers_", i, "_p1_coverages.tsv", sep = ""), col.names = c("c1", "c2"))

  kmer_pairs_p1 = cbind(kmer_pairs_p1, kmer_covs_p1)

  kmer_pairs_p2 = read.table(paste("/mnt/scratch/robe1195/Josephs_Lab_Projects/poolseq-kmers/workflow/", "kmerpairs_", i, "_p2_sequences.tsv", sep = ""), col.names = c("k1"))

  kmer_covs_p2 = read.table(paste("/mnt/scratch/robe1195/Josephs_Lab_Projects/poolseq-kmers/workflow/kmerpairs_", i, "_p2_coverages.tsv", sep = ""), col.names = c("c1", "c2"))

  kmer_pairs_p2 = cbind(kmer_pairs_p2, kmer_covs_p2)

  # calculate minor allele frequency
  kmer_pairs_p1$kmer_maf = pmin(kmer_pairs_p1$c1, kmer_pairs_p1$c2)/(kmer_pairs_p1$c1 + kmer_pairs_p1$c2)
  
  kmer_pairs_p2$kmer_maf = pmin(kmer_pairs_p2$c1, kmer_pairs_p2$c2)/(kmer_pairs_p2$c1 + kmer_pairs_p2$c2)
  
  kmer_pairs_p1p2 = merge(kmer_pairs_p1, kmer_pairs_p2, by = "k1")
  ...
  # compare snp and k-mer mafs
  kmer_pairs_p1p2$leftk = substr(kmer_pairs_p1p2$k1, 1, 15)
  kmer_pairs_p1p2$rightk = substr(kmer_pairs_p1p2$k1, 17, 31)
  ...
  # calculate fst for kmers
  kmer_fst_values = fst(2*75, pos_kmer_pairs$c1.x, pos_kmer_pairs$kmer_maf.x, 2*75, pos_kmer_pairs$c1.y, pos_kmer_pairs$kmer_maf.y, "population")

  pos_kmer_pairs = cbind(pos_kmer_pairs, kmer_fst_values)
  
  # add in SNP fst values
  snp_vs_kmer_fst = merge(true_af[,c("pos", "nei", "hudson")], pos_kmer_pairs, by = "pos")
  
  ggplot(snp_vs_kmer_fst, aes(x = nei.x, y = nei.y)) +
    geom_point() +
    theme_classic()
  
  ggplot(snp_vs_kmer_fst, aes(x = hudson.x, y = hudson.y)) +
    geom_point() +
    theme_classic()
...
ggarrange(tpr_vs_coverage_plot, tnr_vs_coverage_plot, fpr_vs_coverage_plot, fnr_vs_coverage_plot, common.legend = T, font.label = list(size = 16), labels = c("A", "B", "C", "D"), nrow = 2, ncol = 2)

ggsave(paste("../results/", today, "/coverage_vs_tpr_tnr_fpr_fnr.png", sep = ""), unit = "in", width = 8, height = 8)
